...
\small\itshape
``Einstein said that space and time are relative. I say that spacetime itself is
conditional — it does not exist everywhere. And that resolves the contradiction
between General Relativity and Quantum Mechanics.''\\[0.35em]
\upshape\small\hfill — Max Meeuvissen, May 2026
\end{minipage}
\end{center}
\vspace{0.4cm}
...
\paragraph{Reformulated source term (revision of 4 May 2026).}

The original candidate $J(T) = \kappa T/(T+T_0)$ was a positive source
driving $\Phi$ toward 1 in high-energy regions.  On reflection, this is
causally inconsistent: the stress-energy density $T$ is defined only
where $\Phi > 0$ — it presupposes the existence of spacetime.  $T$
cannot therefore be the cause of $\Phi$.  The causal direction must be
reversed: $T$ is a \emph{consequence} of spacetime, not its driver.
\textbf{[Candidate — conceptual revision, Eintrag~012.]}

The reformulated source term treats $T$ as a \emph{destructive} factor:
high mass-energy density drives the system back toward the superstate
(the singularity phase transition), not toward spacetime formation.
\begin{equation}
  J(T) = -\kappa\,\max\!\bigl(0,\,T - T_c\bigr)/T_c,
  \label{eq:source}
\end{equation}
where $T_c$ is the critical energy density (singularity threshold) and
$\kappa > 0$.  Below $T_c$: $J = 0$.
Above $T_c$: $J < 0$, driving $\Phi \to 0$.

\paragraph{Autonomous expansion front.}
Since the cosmic expansion front is not driven by matter (matter is a
product of spacetime, not its cause), the front propagates according to
the source-free equation:
\begin{equation}
  \Box\Phi + V'(\Phi) = 0,
  \label{eq:autonomous}
\end{equation}
the equation of a topological domain wall in the double-well potential
\eqref{eq:potential}.  The front is topologically stable; its
propagation velocity is determined by $\lambda$ and the potential shape,
independent of matter content.  \textbf{[Candidate.]}

\begin{openbox}
\textbf{Open Problem 8 — What determines $T_c$?}\\
$T_c$ is the critical energy density above which spacetime collapses
into the superstate (the singularity threshold).  Candidate scales
include the Planck density
($\rho_{\rm Pl} \approx 5 \times 10^{96}~\text{kg/m}^3$) and
other fundamental scales.  The derivation of $T_c$ from fundamental
constants is the central open problem of the local $\Phi$-dynamics.
\end{openbox}
...
Reformulated source term $J$ (destructive, Eq.~\ref{eq:source})
  & Candidate & Consistent with causal direction\\
Autonomous expansion front Eq.~\eqref{eq:autonomous}
  & Candidate & Formal derivation from action\\
Value of $T_c$ (critical density for singularity)
...
\textbf{Mathematical consequence.}  A boundary tension $\sigma$ (energy per unit
area) gives a boundary energy density $\rho_B = 3\sigma/R$.  Since
$\rho_B \propto 1/R$ decreases with expansion, the equation of state is
$w \approx -1/3$, \emph{not} $w = -1$ as required by observation.  For
$w = -1$, a constant energy density — proportional to the volume
$\propto R^3$ — is needed, not a surface term $\propto R^2$.  Simple
boundary tension therefore does not reproduce the observed dark energy
equation of state.  This is a concrete mathematical challenge for the
...
\begin{openbox}
\textbf{Open Problem 7 — Can the multi-bubble alternative be conceptually sustained?}\\
The speculative multi-bubble variant (Section~\ref{sec:cyclic},
Phase~2) posits that multiple spacetime episodes originate
independently in the superstate and dock at inter-dimensional
boundaries.  A prior conceptual problem must be resolved before
formalization: the superstate, by definition, has no spatial
structure — so what differentiates regions within it that could
give rise to separate bubbles?  Any mechanism that differentiates
superstate regions implicitly introduces a pre-superstate structure,
which would either require justification or contradict the foundational
definition.
\end{openbox}
...
\begin{openbox}
\textbf{Open Problem 13 — Active, passive, or bidirectional expansion?}\\
Is the propagation of the spacetime wavefront into the superstate purely
passive (the substrate expands), actively bidirectional (the superstate
simultaneously attracts the wavefront), or is this dichotomy
conceptually ill-posed?  If bidirectional: does the superstate require
an internal structure (pressure, potential) to exert attraction, and
would this contradict its definition as structure-free?
\end{openbox}

\begin{openbox}
\textbf{Open Problem 14 — Does any boundary mechanism yield $w = -1$?}\\
Simple boundary tension gives $w \approx -1/3$ (Open Problem~5).  For
$w = -1$, the energy density must be constant — independent of the
boundary radius $R$.  This would require the effective dark energy
contribution to grow proportionally to the enclosed volume $\propto
R^3$, not the boundary area $\propto R^2$.  What physical mechanism of
the spacetime-superstate interface, if any, generates a volume-scaling
rather than area-scaling energy?
\end{openbox}
...
Problems~13--14 were identified during the reformulation of the
$\Phi$-source term and the dark energy analysis (Einträge 012--013,
4~May~2026).}
